...
%DIF DEL access_before.tex   Sun Mar 29 02:28:55 2026
%DIF ADD access.tex          Sun Mar 29 20:15:33 2026
\usepackage{cite}
...
\RequirePackage[dvipsnames]{xcolor} %DIF PREAMBLE
\DeclareOldFontCommand{\sf}{\normalfont\sffamily}{\mathsf} %DIF PREAMBLE
% Added text: blue bold (line-break safe; avoids unbreakable \colorbox hbox)
\providecommand{\DIFaddtex}[1]{{\color{blue}\bfseries #1}} %DIF PREAMBLE
% Deleted text: red footnote (avoids ulem/cite conflict)
\providecommand{\DIFdeltex}[1]{{\color{red}[..\footnote{\color{black}Removed: #1}]}} %DIF PREAMBLE
...
\DIFaddbegin \DIFadd{In sectors such as the petrochemical and power generation industries, timely maintenance and replacement of critical mechanical components are of paramount importance~\cite{b16,b25}. However, key components such as industrial compressors are often sourced from a limited number of overseas manufacturers (e.g., Japan, Germany, United States). This reliance introduces high costs, extended lead times, and vulnerability to global supply-chain disruptions, weakening domestic technological competitiveness over time~\cite{b16}. Automated pipelines that convert 3D scans directly into high-precision CAD models can significantly reduce design turnaround and cost in such settings.
}

\DIFaddend We address these gaps with Topology-Constrained Neuro Boundary Representation (NeuroB-Rep), a solver that couples a hybrid geometric representation (primitives $\cup$ implicit signed distance function, SDF) with a topology-aware objective and a contact gating schedule.
...
\begin{itemize}
  ...
  \item \textbf{Industrial case study and reproducible artifact.} A hybrid AI pipeline Cloud2CAD that integrates NeuroB-Rep into an end-to-end workflow from 3D point clouds to watertight STEP B-reps on complex compressor components (shaft, impeller, casing), together with a \DIFdelbegin \DIFdel{planned self-contained artifact (configs, per-run JSON/CSV logs, summary tables, and STEP files)~\cite{b2}}\DIFdelend \DIFaddbegin \DIFadd{public artifact (evaluation scripts, default configuration, and representative output meshes)}\DIFaddend . All ablations are re-evaluated under a \emph{fixed} scale to prevent protocol drift.
\end{itemize}

\DIFdelbegin \DIFdel{In sectors such as the petrochemical and power generation industries, timely maintenance and replacement of critical mechanical components are of paramount importance~\cite{b16,b25}.
However, key componentssuch as industrial compressors are often sourced from a limited number of overseas manufacturers (e.g., Japan, Germany, United States)
. This reliance introduces high costs, extended lead times, and vulnerability to global supply-chain disruptions, weakening domestic technological competitiveness over time~\cite{b16}.
In our industrial study, the resulting Cloud2CAD pipeline automatically converts 3D }\DIFdelend \DIFaddbegin \DIFadd{The remainder of this paper is organized as follows.
Section~\ref{sec:prelim} introduces the B-Rep data model and continuity definitions used throughout.
Section~\ref{sec:related} surveys related work in }\DIFaddend point-cloud \DIFdelbegin \DIFdel{data into high-precision CAD models, achieving average geometric errors on the order of $10^{-2}$\,mm relative to the scan on the reported case and indicating potential reductions in design turnaround and cost in our internal evaluation.
}\DIFdelend \DIFaddbegin \DIFadd{reconstruction, primitive fitting, and B-Rep learning.
...
Section~\ref{sec:artifact} addresses reproducibility and artifact availability.
...
\DIFadd{This section defines the core concepts referenced throughout the paper.
}

\subsection{\DIFadd{Boundary Representation (B-Rep)}}
\DIFadd{A B-Rep encodes a solid model as a hierarchy of topological entities:
...
\subsection{\DIFadd{Surface Continuity}}
\DIFadd{Two adjacent patches sharing an edge can satisfy different orders of continuity:
}\begin{itemize}
  \item \DIFadd{\textbf{$G^0$ (positional) continuity:} the patches meet along the shared edge without gaps.
  }\item \DIFadd{\textbf{$G^1$ (tangent) continuity:} the surface normals of both patches agree along the shared edge, ensuring a smooth visual transition.
  }\item \DIFadd{\textbf{$G^2$ (curvature) continuity:} the principal curvatures match across the edge, yielding a seamless highlight flow; this is typically required for Class-A surfaces.
}\end{itemize}
\DIFadd{In our objective (Sec.~\ref{sec:method}), $E_{G1}$ and $E_{G2}$ penalize deviations from $G^1$ and $G^2$ continuity, respectively.
}

\subsection{\DIFadd{Self-Intersection Repair}}
\DIFadd{Self-intersections arise when two surface patches overlap in 3D space, violating manifoldness.
...
\DIFadd{We organize prior work into four groups and summarize the key
distinctions with our approach in Table~\ref{tab:method_comparison}.
}

\subsection{\DIFadd{Point-Cloud Segmentation}}
\DIFaddend Point-based deep networks process unordered sets effectively and remain a backbone for segmentation-driven reconstruction\DIFaddbegin \DIFadd{~}\DIFaddend \cite{b7,b8}. Recent transformer-based variants further improve boundary sensitivity and multi-scale context\DIFaddbegin \DIFadd{~}\DIFaddend \cite{b23}. We adopt a PointNet-family segmentation stage to obtain per-point labels that seed patch formation (Fig.~\ref{fig:pointnet_overview}). \DIFdelbegin \DIFdel{Over-simplification }\DIFdelend \DIFaddbegin \DIFadd{However, over-simplification }\DIFaddend in complex regions can blur inter-patch boundaries \DIFdelbegin \DIFdel{; an impeller example exhibiting cross-boundary layer misassignment is shown in }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend Fig.~\ref{fig:impeller_misassign}\DIFdelbegin \DIFdel{. For B-rep-aware learning and message passing over CAD structures, see \cite{b3,b9}; for generative B-rep pipelines, see \cite{b1,b4,b20}; and for recovering CAD code }\DIFdelend \DIFaddbegin \DIFadd{), motivating explicit topological control downstream (Fig.~\ref{fig:complex_misassign}).
}

\subsection{\DIFadd{Primitive Fitting and Assembly}}
\DIFadd{RANSAC-based primitive detection~\cite{b15} and supervised fitting~\cite{b27} extract planes, cylinders, cones, and spheres }\DIFaddend from point clouds\DIFdelbegin \DIFdel{, see \cite{b24}. Primitive-centric segmentation/assembly }\DIFdelend \DIFaddbegin \DIFadd{. Li et al.~\cite{b27} train a network to predict primitive parameters in a supervised fashion, achieving high accuracy on clean scans but limited to a fixed set of primitive types without freeform surface support. Structure-aware assembly methods~\cite{b14,b19} }\DIFaddend and parametric surface fitting\DIFdelbegin \DIFdel{are also relevant \cite{b14,b18,b19,b22}. }\DIFdelend \DIFaddbegin \DIFadd{~\cite{b22} refine these primitives into coherent models. PrimitiveNet~\cite{b28} introduces adversarial metric learning for primitive instance segmentation, improving boundary localization but not enforcing inter-surface contacts or continuity. ParSeNet~\cite{b19} jointly segments and fits parametric surfaces but relies on post-hoc assembly without explicit topology constraints. These methods produce accurate individual primitives yet lack a unified objective that enforces inter-surface contacts, $G^1$ continuity, and self-intersection avoidance jointly.
}

\subsection{\DIFadd{B-Rep Learning and Generative Models}}
\DIFadd{BRepNet~\cite{b3} and UV-Net~\cite{b9} introduce message passing over face--edge--vertex graphs but operate on existing B-Reps rather than reconstructing them from scans. Generative pipelines such as ComplexGen~\cite{b1}, BrepGen~\cite{b4}, and Split-and-Fit~\cite{b20} synthesize B-Rep structures from latent codes or point clouds. ComplexGen~\cite{b1} generates B-Rep chain complexes but requires dense ground-truth face--edge--vertex supervision from the ABC dataset and does not optimize for physical contact closure on industrial scans. CAD-Recode~\cite{b24} recovers CSG-like CAD code, offering interpretability at the expense of surface fidelity on complex freeform regions. Maurell et al.~\cite{b31} contribute a large-scale synthetic industrial dataset for benchmarking geometric deep learning in factory scenes, but do not propose a reconstruction method. These approaches advance B-Rep awareness but do not jointly optimize a contact-aware, mm-locked multi-objective energy as we do.
}

\subsection{\DIFadd{Hybrid Reconstruction and Point2CAD}}
\DIFaddend Point2CAD\DIFaddbegin \DIFadd{~\cite{b2} }\DIFaddend is a representative \DIFdelbegin \DIFdel{primitive/implicit fitting }\DIFdelend \DIFaddbegin \DIFadd{hybrid }\DIFaddend pipeline: it segments the point cloud, fits \DIFdelbegin \DIFdel{Random Sample Consensus (RANSAC) based }\DIFdelend \DIFaddbegin \DIFadd{RANSAC-based }\DIFaddend primitives and implicit neural surfaces, and \DIFdelbegin \DIFdel{then }\DIFdelend refines them with a largely distance-centric objective\DIFdelbegin \DIFdel{\cite{b24}}\DIFdelend . Inter-surface contacts and \DIFdelbegin \DIFdel{Tangent Continuity (G1) }\DIFdelend \DIFaddbegin \DIFadd{$G^1$ }\DIFaddend continuity are not encoded as an explicit contact graph with its own schedule, and self-intersection is handled only implicitly through mesh-level post-processing. Our method can \DIFdelbegin \DIFdel{therefore }\DIFdelend be seen as a topology-constrained \DIFdelbegin \DIFdel{continuation }\DIFdelend \DIFaddbegin \DIFadd{extension }\DIFaddend of this line of work: we reuse a similar hybrid surface representation but replace the optimization core with a contact-aware, mm-locked multi-objective solver that explicitly tracks and regularizes the contact graph.

\DIFdelbegin %DIFDELCMD < \begin{figure}[t]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\linewidth]{fig1.jpg}
%DIFDELCMD <   \caption{Overview of a PointNet-family semantic segmentation pipeline used to obtain per-point labels that guide patch formation \cite{b7,b8}.}
%DIFDELCMD <   \label{fig:pointnet_overview}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Over-simplification in complex regions motivates explicit topological control:
...
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\linewidth]{fig2.png}
%DIFDELCMD <   \caption{Prior-work pipeline (Point2CAD \cite{b2}): overview of the transformation stages $\text{xyzc} \rightarrow \text{ply} \rightarrow \text{ply clipped}$ .}
%DIFDELCMD <   \label{fig:point2cad_overview}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Conventional Reverse Engineering Methods}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend Traditional reverse engineering \DIFdelbegin \DIFdel{(RE) }\DIFdelend relies on manual measurement and skilled modeling \DIFdelbegin \DIFdel{. CAD operators model parts from caliper/gauge measurements; for complex shapes, CMMs acquire discrete surface samples that are then modeled in CAD }\DIFdelend \DIFaddbegin \DIFadd{using calipers, gauges, and coordinate measuring machines (CMMs)~}\DIFaddend \cite{b16,b25}. While accurate in expert hands, these workflows are \DIFdelbegin \DIFdel{labor intensive, slow }\DIFdelend \DIFaddbegin \DIFadd{labor-intensive, }\DIFaddend often requiring days or \DIFdelbegin \DIFdel{even weeksto complete, and operator dependent}\DIFdelend \DIFaddbegin \DIFadd{weeks, and operator-dependent}\DIFaddend ~\cite{b16}. \DIFdelbegin \DIFdel{This motivates }\DIFdelend \DIFaddbegin \DIFadd{Liu~\cite{b16} proposes an adaptive process but still requires substantial manual intervention. This motivates fully }\DIFaddend automated pipelines with consistent accuracy and faster turnaround.
...
\DIFdelbegin \DIFdel{Patches form a B-rep candidate graph with face–edge–vertex incidence; edges at patch interfaces are potential contacts. }\DIFdelend Our hybridization follows robust primitive detection\DIFaddbegin \DIFadd{~}\DIFaddend \cite{b15} and implicit function modeling\DIFaddbegin \DIFadd{~}\DIFaddend \cite{b5,b6,b17,b21}, with classical radial-basis-function reconstruction as a related implicit alternative\DIFaddbegin \DIFadd{~}\DIFaddend \cite{b13}.

\subsubsection{Hybrid Fitting with INRs and Primitives}
Following Point2CAD, simple patches use \DIFdelbegin \DIFdel{RANSAC based primitives while free-form }\DIFdelend \DIFaddbegin \DIFadd{RANSAC-based primitives while freeform }\DIFaddend regions adopt an INR\DIFaddbegin \DIFadd{~}\DIFaddend \cite{b2,b15,b5,b17,b6,b21}. For clarity, Fig.~\ref{fig:point2cad_overview} summarizes the prior pipeline stages ($\text{xyz} \rightarrow \text{xyzc} \rightarrow \text{ply}$).

\DIFaddbegin \subsubsection{\DIFadd{From Points to a Patch Graph}}
\label{sec:patch_graph}
\DIFadd{Given the per-point segmentation labels, each label group defines a \emph{face} in the candidate B-Rep.
...
\DIFadd{Table~\ref{tab:pipeline_comparison} summarizes the key differences between our pipeline and Point2CAD.
}
...
\DIFaddend \subsection{Contact-Aware Objective}
...
\DIFdelbegin \DIFdel{Let }\DIFdelend \DIFaddbegin \DIFadd{We define a \emph{projection operator} }\DIFaddend $\Pi_j(\cdot)$ \DIFdelbegin \DIFdel{be the projection operator onto patch}\DIFdelend \DIFaddbegin \DIFadd{that maps an
arbitrary 3D point to the closest point on patch~}\DIFaddend $j$\DIFdelbegin \DIFdel{(analytic projection for primitives ; gradient-based projection }\DIFdelend \DIFaddbegin \DIFadd{.
For analytic primitives (planes, cylinders, etc.), $\Pi_j$ has a
closed-form expression; }\DIFaddend for the INR SDF\DIFdelbegin \DIFdel{), and define
symmetric distances }\DIFdelend \DIFaddbegin \DIFadd{, we use gradient descent
on the implicit function to find the nearest on-surface point.
}

\DIFadd{Given a boundary sample pair $(\mathbf{p},\mathbf{q})$ with $\mathbf{p}$
on patch~$i$ and $\mathbf{q}$ on patch~$j$, we compute two
\emph{cross-projection distances}:
}\DIFaddend $d_{ij}=\|\mathbf{p}-\Pi_j(\mathbf{p})\|$\DIFaddbegin \DIFadd{, measuring how far
$\mathbf{p}$ lies from patch~$j$, }\DIFaddend and
$d_{ji}=\|\mathbf{q}-\Pi_i(\mathbf{q})\|$\DIFdelbegin \DIFdel{for paired samples $(\mathbf{p},\mathbf{q})$.
With the hinge }\DIFdelend \DIFaddbegin \DIFadd{, the symmetric counterpart.
}

\DIFadd{We penalize non-zero gaps using a hinge function
}\DIFaddend $h_\varepsilon(d)=\max(0,d-\varepsilon)^2$, \DIFdelbegin \DIFdel{our gap term }\DIFdelend \DIFaddbegin \DIFadd{where $\varepsilon>0$ is a
contact tolerance (in mm). The gap energy }\DIFaddend is
\begin{equation}
E_{\mathrm{gap}}
...
\DIFaddbegin \DIFadd{Note that $d_{ij,k}$ measures how far a boundary sample on patch~$i$ lies from the surface of patch~$j$, whereas the symmetric quantity $s_{ij,k}$ defined below measures the distance between the two \emph{projected} points. }\DIFaddend For convenience, let
\begin{equation}
s_{ij,k}
:= \bigl\|\Pi_j(\mathbf{p}^{(i)}_k)
      - \Pi_i(\mathbf{p}^{(j)}_k)\bigr\|.
\end{equation}
Then the self-intersection suppressor is a soft barrier \DIFaddbegin \DIFadd{(a smooth penalty that grows rapidly as two surfaces approach overlap, approximating the hard constraint $s>0$ without introducing non-differentiable jumps)
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[t]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\linewidth]{fig6.jpg}
%DIFDELCMD <   \caption{Layer-wise guidance: (a) input cloud; (b) boundary extraction; (c) layer map; (d) reconstructed model.}
%DIFDELCMD <   \label{fig:layer_guidance}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
...
\DIFaddbegin \DIFadd{The purpose of this normalization step is to make the solver invariant
...
\DIFaddend We normalize scans to a canonical pose:
\begin{enumerate}
...
\end{enumerate}
\DIFdelbegin \DIFdel{We release scripts that reproduce this export pipeline with the same tolerance settings as used in our experiments.}\DIFdelend \DIFaddbegin \DIFadd{The evaluation scripts and default configuration released in the public artifact (Sec.~\ref{sec:artifact}) allow independent verification of the metric protocol and hyperparameter settings used in our experiments.}\DIFaddend
...
Absolute metrics quantify geometric \emph{deviation to the design CAD (GT--CAD)} in the fixed datum frame without Iterative Closest Point (ICP). Relative metrics are computed after \DIFdelbegin \DIFdel{point-to-plane }\DIFdelend \DIFaddbegin \DIFadd{point-to-point }\DIFaddend ICP alignment to the raw scan and reported as scan-consistency measures. For both tracks, we report the mean and 95\% confidence intervals estimated over repeated re-sampling/re-seeding runs (Sec.~\ref{sec:robustness}); we do \emph{not} claim metrologically traceable uncertainty.
...
For relative evaluation, we register $Y$ to the raw scan $X_{\mathrm{Scan}}$ by \DIFdelbegin \DIFdel{point-to-plane }\DIFdelend \DIFaddbegin \DIFadd{point-to-point }\DIFaddend ICP and compute
\[
...
We establish a world frame \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend the GT--CAD datum scheme \DIFdelbegin \DIFdel{(ISO}\DIFdelend \DIFaddbegin \DIFadd{following ISO~}\DIFaddend 5459\DIFdelbegin \DIFdel{). }\DIFdelend \DIFaddbegin \DIFadd{~\cite{b16}.
...
}\DIFaddend A single global scale factor is sanity-checked using a simple gauge object (ruler/block)
...
\DIFaddbegin \paragraph*{\DIFadd{Rationale for hybrid evaluation.}}
\label{sec:hybrid_eval_rationale}
\DIFadd{This hybrid protocol is an intentional design choice to decouple
...
\DIFaddend \subsubsection{Evaluation Metrics}
...
\begin{itemize}
  ...
  \item \textbf{Relative Position Accuracy (mm):} After centroid alignment, perform ICP (\DIFdelbegin \DIFdel{point-to-plane}\DIFdelend \DIFaddbegin \DIFadd{point-to-point}\DIFaddend ) with max correspondence distance \(\delta_{\mathrm{ICP}}\) and at most \(I_{\mathrm{ICP}}\) iterations. Report symmetric Chamfer mean \DIFaddbegin \DIFadd{(Eq.~\eqref{eq:chamfer}) }\DIFaddend and Hausdorff max \DIFaddbegin \DIFadd{(Eq.~\eqref{eq:hausdorff})}\DIFaddend :
\end{itemize}

\DIFdelbegin %DIFDELCMD < \begin{align}
%DIFDELCMD < d_{\mathrm{CD}}(X,Y)
%DIFDELCMD <   &= \frac{1}{|X|}\sum_{x\in X}\min_{y\in Y}\|x-y\|_2 \notag\\
%DIFDELCMD <   &\quad + \frac{1}{|Y|}\sum_{y\in Y}\min_{x\in X}\|y-x\|_2,
%DIFDELCMD <   \label{eq:chamfer} 
%DIFDELCMD < \\[2pt]
%DIFDELCMD < d_{\mathrm{HD}}(X,Y)
%DIFDELCMD <   &= \max\Big\{
%DIFDELCMD <         \max_{x\in X}\min_{y\in Y}\|x-y\|_2, \notag\\
%DIFDELCMD <   &\qquad\quad
%DIFDELCMD <         \max_{y\in Y}\min_{x\in X}\|y-x\|_2
%DIFDELCMD <      \Big\}.
%DIFDELCMD <   \label{eq:hausdorff}
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
...
\begin{itemize}
  ...
  \item \textbf{Surface Coincidence (mm):} Unidirectional mean \DIFdelbegin \DIFdel{distance }\DIFdelend \DIFaddbegin \DIFadd{and max distances }\DIFaddend from reference to result, i.e., the \DIFdelbegin \DIFdel{first }\DIFdelend \DIFaddbegin \DIFadd{mean and max of $\min_{y\in Y}\|x-y\|_2$ over $x\in X$ (first directional }\DIFaddend term of Eq.~\eqref{eq:chamfer}\DIFaddbegin \DIFadd{)}\DIFaddend .
  \item \DIFaddbegin \DIFadd{\textbf{Shape Deviation (mm):} Symmetric mean Chamfer distance (Eq.~\eqref{eq:chamfer}); equivalent to Geometric Accuracy (Mean) and reported separately to highlight overall shape fidelity as a single scalar.
  }\item \DIFaddend \textbf{Specific Feature Accuracy (mm):} On function-critical regions of interest (ROIs) (e.g., impeller blade cross-sections), extract a \SI{0.5}{\milli\meter}-thick slice on a prescribed plane, project to 2D, remove the top 5\% outliers, and report robust mean error.
\end{itemize}
\textbf{Settings.} Unless stated otherwise, \(N=M\) uniform samples per surface; \DIFdelbegin \DIFdel{point-to-plane ICP with \(\delta_{\mathrm{ICP}}=\SI{0.5}{\milli\meter}\)}\DIFdelend \DIFaddbegin \DIFadd{point-to-point ICP with \(\delta_{\mathrm{ICP}}=\SI{1.0}{\milli\meter}\)}\DIFaddend , \(I_{\mathrm{ICP}}=50\).
...
device \DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{(NVIDIA, CUDA).
...
The }\DIFaddend contact-aware optimizer performs bounded snaps every
...
\DIFaddbegin \subsection{\DIFadd{Computational Cost}}
\label{sec:cost}
\DIFadd{Table~\ref{tab:cost} reports wall-clock time and peak GPU memory for a
...
\DIFadd{The $\sim$75\% overhead relative to Point2CAD is attributable to the
...
\DIFaddend \subsection{Monte-Carlo Robustness to Sampling and Seeding}
...
ICP with \DIFdelbegin \DIFdel{$\delta_{\mathrm{ICP}}{=}\SI{0.5}{\milli\meter}$ }\DIFdelend \DIFaddbegin \DIFadd{$\delta_{\mathrm{ICP}}{=}\SI{1.0}{\milli\meter}$ }\DIFaddend and $I_{\mathrm{ICP}}{=}50$). % see Sec. IV-B
...
\DIFaddbegin \subsection{\DIFadd{Effect of PCA Canonical Transform}}
\label{sec:pca_ablation}
\DIFadd{Table~\ref{tab:pca_ablation} isolates the effect of the PCA normalization
...
\DIFaddend \subsection{Qualitative Examples}
...
fixed relative scale, identical sampling ($N{=}M$ per surface), and \DIFdelbegin \DIFdel{point-to-plane ICP with
$\delta_{\mathrm{ICP}}{=}\SI{0.5}{\milli\meter}$ }\DIFdelend \DIFaddbegin \DIFadd{point-to-point ICP with
$\delta_{\mathrm{ICP}}{=}\SI{1.0}{\milli\meter}$ }\DIFaddend and $I_{\mathrm{ICP}}{=}50$.
...
(fixed relative contact-gap scale; $N{=}M$ uniform sampling; \DIFdelbegin \DIFdel{point-to-plane ICP with
$\delta_{\mathrm{ICP}}{=}\SI{0.5}{\milli\meter}$}\DIFdelend \DIFaddbegin \DIFadd{point-to-point ICP with
$\delta_{\mathrm{ICP}}{=}\SI{1.0}{\milli\meter}$}\DIFaddend , $I_{\mathrm{ICP}}{=}50$; mm metrics after world mapping).
For robustness, we report the average over \textbf{\DIFdelbegin \DIFdel{1}%DIFDELCMD < {%%%
\DIFdel{,}%DIFDELCMD < }%%%
\DIFdel{000}\DIFdelend \DIFaddbegin \DIFadd{120}\DIFaddend } independent repeats
\DIFaddbegin \DIFadd{per component }\DIFaddend (re-sampling and re-seeding), as $\bar x \pm 95\%$ CI, and release all raw CSV/JSON and the
aggregation script in the artifact (Sec.~\ref{sec:artifact}).

\paragraph*{Protocol harmonization.}
All symmetric distances (Chamfer) are computed as the \emph{\DIFdelbegin \DIFdel{sum }\DIFdelend \DIFaddbegin \DIFadd{mean }\DIFaddend of the two directional means}
(symmetric \DIFdelbegin \DIFdel{sum}\DIFdelend \DIFaddbegin \DIFadd{average}\DIFaddend ), consistent with Eq.~\eqref{eq:chamfer}.
Registration uses \emph{\DIFdelbegin \DIFdel{point-to-plane}\DIFdelend \DIFaddbegin \DIFadd{point-to-point}\DIFaddend } ICP with a maximum correspondence distance
\DIFdelbegin \DIFdel{$\delta_{\mathrm{ICP}}=\SI{0.5}{\milli\meter}$ }\DIFdelend \DIFaddbegin \DIFadd{$\delta_{\mathrm{ICP}}=\SI{1.0}{\milli\meter}$ }\DIFaddend and at most $I_{\mathrm{ICP}}=50$ iterations.
...
Table~\ref{tab:consistency_all}} after \DIFdelbegin \DIFdel{point-to-plane }\DIFdelend \DIFaddbegin \DIFadd{point-to-point }\DIFaddend ICP alignment.
...
%DIFDELCMD <     round-mode            = places,
%DIFDELCMD <     round-precision       = 4,
%DIFDELCMD <     table-format          = 1.5
%DIFDELCMD <   }
%DIFDELCMD <   \caption{Relative consistency to raw scan after ICP.
%DIFDELCMD <   All distance-based metrics are reported in (mm).}
%DIFDELCMD <   \label{tab:consistency_all}
%DIFDELCMD <   \begin{tabular}{
%DIFDELCMD <       >{\raggedright\arraybackslash}p{0.36\columnwidth}
%DIFDELCMD <       S S S S
%DIFDELCMD <   }
%DIFDELCMD <     \toprule
%DIFDELCMD <     \textbf{Metric} & \textbf{Mean} & \textbf{Std} & \textbf{Median} & \textbf{P95} \\
%DIFDELCMD <     \midrule
%DIFDELCMD <     \multicolumn{5}{l}{\textbf{Impeller}} \\
%DIFDELCMD <     Relative Position (Mean Err)   & 0.00711 & 0.00700 & 0.00713 & 0.00720 \\
%DIFDELCMD <     Relative Position (Max Err)    & 0.02795 & 0.00236 & 0.02868 & 0.03085 \\
%DIFDELCMD <     Length (Width)                 & 0.00321 & 0.00185 & 0.00337 & 0.00678 \\
%DIFDELCMD <     Length (Depth)                 & 0.00032 & 0.00020 & 0.00045 & 0.00053 \\
%DIFDELCMD <     Length (Height)                & 0.01423 & 0.00617 & 0.01468 & 0.02295 \\
%DIFDELCMD <     Geometric Acc (Mean)           & 0.00996 & 0.00008 & 0.00998 & 0.01007 \\
%DIFDELCMD <     Geometric Acc (Max)            & 0.03518 & 0.00248 & 0.03455 & 0.04046 \\
%DIFDELCMD <     Surface Coincidence (Mean)     & 0.00992 & 0.00007 & 0.00992 & 0.01003 \\
%DIFDELCMD <     Surface Coincidence (Max)      & 0.03359 & 0.00226 & 0.03314 & 0.03823 \\
%DIFDELCMD <     Shape Deviation                & 0.00996 & 0.00800 & 0.00998 & 0.01007 \\
%DIFDELCMD <     \midrule
%DIFDELCMD <     \multicolumn{5}{l}{\textbf{Shaft}} \\
%DIFDELCMD <     Relative Position (Mean Err)      & 0.00304 & 0.00002 & 0.00303 & 0.00310 \\
%DIFDELCMD <     Relative Position (Max Err)       & 0.01204 & 0.00263 & 0.01122 & 0.01962 \\
%DIFDELCMD <     Length (Width)                    & 0.00311 & 0.00158 & 0.00315 & 0.00583 \\
%DIFDELCMD <     Length (Depth)                    & 0.00630 & 0.00567 & 0.00405 & 0.01834 \\
%DIFDELCMD <     Length (Height)                   & 0.00947 & 0.00657 & 0.00826 & 0.02107 \\
%DIFDELCMD <     Geometric Acc (Mean)              & 0.00495 & 0.00065 & 0.00475 & 0.00702 \\
%DIFDELCMD <     Geometric Acc (Max)               & 0.01652 & 0.00358 & 0.01544 & 0.02646 \\
%DIFDELCMD <     Surface Coincidence (Mean)        & 0.00496 & 0.00068 & 0.00475 & 0.00707 \\
%DIFDELCMD <     Surface Coincidence (Max)         & 0.01592 & 0.00365 & 0.01481 & 0.02588 \\
%DIFDELCMD <     Shape Deviation (Scan)            & 0.00495 & 0.00065 & 0.00475 & 0.00702 \\
%DIFDELCMD <     \midrule
%DIFDELCMD <     \multicolumn{5}{l}{\textbf{Casing}} \\
%DIFDELCMD <     Relative Position (Mean Err) & 0.00692 & 0.00600 & 0.00691 & 0.00703 \\
%DIFDELCMD <     Relative Position (Max Err)  & 0.02546 & 0.00137 & 0.02520 & 0.02828 \\
%DIFDELCMD <     Length (Width)               & 0.01080 & 0.00740 & 0.01009 & 0.02597 \\
%DIFDELCMD <     Length (Depth)               & 0.00983 & 0.00730 & 0.00851 & 0.02492 \\
%DIFDELCMD <     Length (Height)              & 0.02283 & 0.01412 & 0.02269 & 0.04608 \\
%DIFDELCMD <     Geometric Acc (Mean)         & 0.00992 & 0.00009 & 0.00992 & 0.01006 \\
%DIFDELCMD <     Geometric Acc (Max)          & 0.03498 & 0.00234 & 0.03486 & 0.03934 \\
%DIFDELCMD <     Surface Coincidence (Mean)   & 0.00992 & 0.00909 & 0.00992 & 0.01008 \\
%DIFDELCMD <     Surface Coincidence (Max)    & 0.03335 & 0.00204 & 0.03312 & 0.03697 \\
%DIFDELCMD <     Shape Deviation (Scan)       & 0.00992 & 0.009090 & 0.00992 & 0.01006 \\
...
\DIFaddbegin \paragraph*{\DIFadd{Baseline comparison with Point2CAD.}}
\DIFadd{Table~\ref{tab:consistency_all} places our results alongside Point2CAD~\cite{b2}
...
\DIFaddend \section{Reproducibility and Artifact}
\label{sec:artifact}
\DIFdelbegin \DIFdel{To support reproducibility, we built an artifact containing all code,
...
ablation summary and all reported plots directly from the stored outputs.}\DIFdelend \DIFaddbegin \DIFadd{Due to corporate confidentiality and intellectual property constraints
...
}\url{https://github.com/Juunary/TopoConstrained-NeuroBrep}\DIFadd{
containing:
}\begin{enumerate}
  \item \textbf{\DIFadd{Evaluation scripts.}} \DIFadd{Standalone Python scripts that compute
        ...
        Dependencies are limited to }\texttt{\DIFadd{numpy}}\DIFadd{, }\texttt{\DIFadd{trimesh}}\DIFadd{,
        and }\texttt{\DIFadd{scipy}}\DIFadd{; no proprietary packages are required.}
  \item \textbf{\DIFadd{Default configuration file.}}
        \texttt{\DIFadd{configs/default.yaml}} \DIFadd{lists every hyperparameter reported
  ...
  \item \textbf{\DIFadd{Representative output meshes.}} \DIFadd{Final reconstructed PLY files
  ...
  \item \textbf{\DIFadd{Numerical summary.}} \DIFadd{A CSV file recording the
        Table~\ref{tab:consistency_all} values for the Impeller component,
        produced by a single evaluation run.}
\end{enumerate}
\DIFadd{The evaluation scripts depend only on publicly available packages
(}\texttt{\DIFadd{numpy}}\DIFadd{, }\texttt{\DIFadd{trimesh}}\DIFadd{, }\texttt{\DIFadd{scipy}}\DIFadd{) and require no
proprietary dependencies.
The remaining solver code and the full proprietary scan data are
available upon reasonable request to the corresponding author.}\DIFaddend
...
Table\DIFdelbegin \DIFdel{s}\DIFdelend  ~\ref{tab:consistency_all} reveals that solver energy and geometric
...
\DIFaddbegin \subsection{\DIFadd{Scope of Baseline Comparisons}}
\DIFadd{Among the methods surveyed in Table~\ref{tab:method_comparison}, only
...
\begin{enumerate}
  \item \DIFadd{\textbf{Dataset mismatch.}
  ...
  }\item \DIFadd{\textbf{Output-format disparity.}
  ...
  }\item \DIFadd{\textbf{Evaluation-metric incompatibility.}
...
\DIFadd{Point2CAD shares the same RANSAC-seeded primitive fitting plus neural
...
\DIFaddend \subsection{Linking Protocol and Claims}
Our evaluation protocol fixes the contact-gap scale while reporting geometry in world coordinates (mm).
For the case-wise metrics in Table\DIFdelbegin \DIFdel{s}\DIFdelend  ~\ref{tab:consistency_all}, the reference geometry is a numerically defined PLY mesh (the current artifact), and all Chamfer/Hausdorff distances are computed in code by comparing the reconstructed meshes against this fixed reference.
...
As summarized in Table\DIFdelbegin \DIFdel{s}\DIFdelend  ~\ref{tab:consistency_all}, mm-level geometry (Chamfer/Hausdorff and related measures) meets the thresholds
...
qualitative boundary preservation in Figs.~\ref{fig:impeller_misassign}\DIFdelbegin \DIFdel{–}\DIFdelend \DIFaddbegin \DIFadd{--}\DIFaddend \ref{fig:layer_guidance}. These trends align with observations reported in primitive assembly and parametric fitting\DIFaddbegin \DIFadd{~}\DIFaddend \cite{b14,b19}.

\DIFaddbegin \subsection{\DIFadd{Failure Modes and Mitigations}}
\label{sec:failure_modes}
\DIFadd{The cross-boundary misassignment illustrated in Figs.~\ref{fig:impeller_misassign} and~\ref{fig:complex_misassign} represents the primary failure mode observed in our experiments. This misassignment originates in the upstream PointNet segmentation stage: when two geometrically similar surfaces (e.g., adjacent blade fillets on an impeller) share nearly identical local features, the segmentation network may assign them to the same label, merging distinct patches into one.
}

\DIFadd{The downstream effect is twofold. First, the merged patch must represent a region with mixed curvature, degrading the data-term fit (higher $E_{\text{data}}$). Second, the contact graph loses an edge that should exist between the two surfaces, so the gap and $G^1$ terms cannot enforce proper closure at that boundary.
}

\DIFadd{Several mitigations can address this failure mode:
}\begin{itemize}
  \item \DIFadd{\textbf{Adaptive layer thresholds.} Dynamically splitting patches whose residual exceeds a threshold during optimization, creating new contact edges on the fly.
  }\item \DIFadd{\textbf{Boundary-aware resampling.} Using the curvature gradient across the merged region to detect latent boundaries and re-segment locally, without re-running the full PointNet inference.
  }\item \DIFadd{\textbf{Multi-scale segmentation.} Employing a hierarchical segmentation that captures fine-grained boundaries missed by the single-scale PointNet, as suggested by recent transformer-based approaches~\cite{b23}.
}\end{itemize}
\DIFadd{These directions are left for future work.
}

\DIFaddend \section{Threats to Validity and Limitations}
...
per-component 80/20 split (Sec.~\DIFdelbegin \DIFdel{IV‑A}\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:setup}}\DIFaddend ). While representative of our
...
the acceptance criteria in Sec.~\DIFdelbegin \DIFdel{IV‑B }\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:accept} }\DIFaddend and reduces surprises at computer-aided manufacturing (CAM) and quality assurance (QA). Instant mesh self-intersection repair tools can also aid diagnostics \cite{b12}.
...
\textbf{Reproducibility \DIFdelbegin \DIFdel{hygiene}\DIFdelend \DIFaddbegin \DIFadd{protocol}\DIFaddend .} For reproducibility, we \DIFdelbegin \DIFdel{advise
pinning the environment (hash seeds, dependency lockfile), emitting
per‑run }\DIFdelend \DIFaddbegin \DIFadd{fix all random seeds, lock software dependencies, emit per-run }\DIFaddend JSON/CSV summaries, and \DIFdelbegin \DIFdel{recomputing }\DIFdelend \DIFaddbegin \DIFadd{recompute }\DIFaddend all metrics under the fixed scale before aggregation. Our experiments \DIFdelbegin \DIFdel{already follows }\DIFdelend \DIFaddbegin \DIFadd{follow }\DIFaddend this practice for Table~\ref{tab:consistency_all} and Fig.~\ref{fig:learning_gaps_composite}(a)\DIFdelbegin \DIFdel{–}\DIFdelend \DIFaddbegin \DIFadd{--}\DIFaddend (c)\DIFaddbegin \DIFadd{. Due to proprietary industrial data constraints, the full artifact (including raw scans) cannot be made publicly available; however, the solver code and evaluation scripts are available upon reasonable request to the corresponding author
}\DIFaddend 


\section{Conclusion}
\DIFaddbegin \label{sec:conclusion}
\DIFaddend In this work, we introduced Topology-Constrained Neuro boundary representations (\DIFdelbegin \DIFdel{NeuroB-reps}\DIFdelend \DIFaddbegin \DIFadd{NeuroB-Reps}\DIFaddend ), a contact-aware optimization framework to reconstruct watertight, manufacturable CAD models from raw point clouds. By integrating analytic primitives with neural implicit surfaces under topological constraints, our method reliably preserves inter-surface contacts and ensures valid \DIFdelbegin \DIFdel{B-rep }\DIFdelend \DIFaddbegin \DIFadd{B-Rep }\DIFaddend topology. Experimental validation on industrial components \DIFdelbegin \DIFdel{, specifically an }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend impeller, shaft, and casing\DIFdelbegin \DIFdel{, demonstrates that NeuroB-reps achieves geometric accuracy within }\DIFdelend \DIFaddbegin \DIFadd{) demonstrates geometric accuracy in }\DIFaddend the 0.01\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{--0.03\,}\DIFaddend mm range relative to input scans while consistently exporting watertight STEP files. \DIFdelbegin \DIFdel{To support reproducibility and facilitate independent verification, we have detailed the development process and specifications, and presented the resulting CAD models}\DIFdelend \DIFaddbegin \DIFadd{A qualitative comparison with representative methods (Table~\ref{tab:method_comparison}) and a stage-by-stage comparison with Point2CAD (Table~\ref{tab:pipeline_comparison}) confirm that the explicit contact graph, $G^1$/$G^2$ objective, and temperature-gated schedule are the key differentiating contributions}\DIFaddend .

Our study further highlights that solver energy and geometric distances (e.g., Chamfer or Hausdorff) measure complementary aspects of reconstruction quality; optimizing solely for scan-space proximity often neglects the topological validity required for downstream engineering. \DIFdelbegin \DIFdel{Consequently, our approach offers a practical bridge for reverse engineering workflows, yielding models suitable for CAM and QA processes.}\DIFdelend \DIFaddbegin \DIFadd{We also identified the primary failure mode---cross-boundary misassignment from upstream segmentation---and proposed concrete mitigations including adaptive patch splitting and boundary-aware resampling (Sec.~\ref{sec:failure_modes}). }\DIFaddend These findings motivate future work on multi-objective scheduling that jointly balances contact closure and free\DIFdelbegin \DIFdel{-}\DIFdelend form fidelity, extending evaluation to broader part families, and \DIFdelbegin \DIFdel{standardizing mm-locked error reporting}\DIFdelend \DIFaddbegin \DIFadd{incorporating GD\&T-driven objectives}\DIFaddend . Overall, \DIFdelbegin \DIFdel{NeuroB-reps }\DIFdelend \DIFaddbegin \DIFadd{NeuroB-Reps }\DIFaddend complements recent trends in \DIFdelbegin \DIFdel{B-rep learning}\DIFdelend \DIFaddbegin \DIFadd{B-Rep learning~}\DIFaddend \cite{b3,b1,b4,b20} and implicit reconstruction\DIFaddbegin \DIFadd{~}\DIFaddend \cite{b5,b6,b17,b21} by explicitly addressing the rigorous demands of industrial applicability.

\DIFdelbegin %DIFDELCMD < \begin{thebibliography}{26}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \section*{\DIFadd{Acknowledgment}}
\DIFadd{This work was supported by the National Research Foundation of Korea (NRF) grant funded by the Korea government (MSIT) (No.\ RS-2023-00240936).
}\DIFaddend 

\DIFaddbegin \begin{thebibliography}{31}

\DIFaddend \bibitem{b1}
...
\bibitem{b31}
\DIFadd{I. P. Maurell, P. L. Cor\c{c}aque, C. L. Froes, J. F. S. S. Lemos, F. G. Oliveira, and P. Drews,
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{a1.png}}]{JuneWoo Kang}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{a2.png}}]{Chaehwan Lee}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{a4.jpg}}]{Seongmin Yang}
