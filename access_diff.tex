...
\begin{itemize}
  ...
  \item \textbf{Industrial case study and reproducible artifact.} A hybrid AI pipeline Cloud2CAD that integrates NeuroB-Rep into an end-to-end workflow from 3D point clouds to watertight STEP B-reps on complex compressor components (shaft, impeller, casing), together with a \DIFdelbegin \DIFdel{planned }\DIFdelend self-contained artifact (configs, per-run JSON/CSV logs, summary tables, and STEP files)\DIFdelbegin \DIFdel{~\cite{b2}}\DIFdelend . All ablations are re-evaluated under a \emph{fixed} scale to prevent protocol drift.
\end{itemize}
...
and
evaluation configurations, per-run summaries for all experiments on the 
impeller,
shaft, }\DIFdelend \DIFaddbegin \DIFadd{Due to corporate confidentiality and intellectual property constraints
...
\DIFadd{containing:
  ...
  \item \DIFadd{\textbf{Evaluation scripts.} Standalone Python scripts that compute
  ...
  }\item \DIFadd{\textbf{Default configuration file.}
  ...
  }\item \DIFadd{\textbf{Representative output meshes.} Final reconstructed PLY files
        for the Impeller component produced by NeuroB-Rep }\DIFaddend and \DIFdelbegin \DIFdel{casing in tables~\ref{tab:consistency_all} }\DIFdelend \DIFaddbegin \DIFadd{Point2CAD,
        together with the reference mesh, enabling direct metric
        reproduction via the provided evaluation script.
  }\item \DIFadd{\textbf{Numerical summary.} A CSV file recording the
...
(\texttt{numpy}}\DIFaddend , \DIFdelbegin \DIFdel{and scripts that recompute all metrics 
under the fixed evaluation scale and regenerate Fig.
  ~\ref{fig:learning_gaps_composite}(a)–(c).
  In practice,
        a 
single convenience script will runthe full evaluation pipeline and reproduce the 
ablation summary and all reported plots directly from the stored outputs}\DIFdelend \DIFaddbegin \DIFadd{\texttt{trimesh}, \texttt{scipy}) and require no
proprietary dependencies}\DIFaddend .
