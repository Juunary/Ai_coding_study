#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# python evaluate_shaft_batch.py   --path1 "../assets/ply/shaft/ply_validation/shaft.ply"  --path2 "../assets/ply/shaft/ply_test"  --start 1 --end 120   --pattern "mesh_{:03d}.ply" --scale 1 --log "evaluation_shaft.log" 
import os
import argparse
import numpy as np
...
def main():
    ap = argparse.ArgumentParser(description="Batch evaluate shaft meshes")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    default_log_path = os.path.join(script_dir, "evaluation_shaft.log")
    ap.add_argument("--path1", help="기준(원본) PLY 파일 경로", default="/mnt/nas4/junewookang/point2cad/assets/shaft/ply_validation/shaft.ply")
    ap.add_argument("--path2", help="mesh_###.ply들이 있는 폴더", default="/mnt/nas4/junewookang/point2cad/assets/shaft/ply_test")
    ap.add_argument("--start", type=int, default=1)
    ap.add_argument("--end",   type=int, default=120)
    ap.add_argument("--pattern", default="shaft_{:03d}.ply")
    ...
    for i in range(args.start, args.end + 1):
        ...
        try:
            ...
            for metric, (val, thr) in criteria.items():
                ...
                if val > thr:
                    fail_notes.append(f"- {metric}: {val:.6f} (기준 {thr})")
                    is_fail = True
