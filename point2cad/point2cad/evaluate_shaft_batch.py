...
def main():
    ...
    ap.add_argument("--path1", required=True, help="기준(원본) PLY 파일 경로")
    ap.add_argument("--path2", required=True, help="mesh_###.ply들이 있는 폴더")
    ap.add_argument("--start", type=int, default=1)
    ap.add_argument("--end",   type=int, default=120)
    ap.add_argument("--pattern", default="mesh_{:03d}.ply")
    ...
    ap.add_argument("--log", default="evaluation_shaft.log")
    args = ap.parse_args()
    ...
    for i in range(args.start, args.end + 1):
        ...
        try:
            ...
            head = [
                "====================",
                f"Mesh1: {os.path.basename(path1)}",
                f"Mesh2: {os.path.basename(path2)}",
                f"평가 시각: {now}",
            ]
            out_lines = head + rows
