...
def process_one_surface(label, points, labels, cfg, device):
    ...
    # ========================== fitting inrs=====================
    recon_inr_shapes = {"is_good_fit": False, "err": np.inf}
    ...
    cone_mesh = visualize_basic_mesh("cone", in_points, recon_basic_shapes, device=device)

    if cone_mesh is None or len(cone_mesh.vertices) == 0:
        cone_err = np.inf
    ...
    if pred_shape == 0:  # plane
        ...
        cylinder_err = recon_basic_shapes["cylinder_err"]
        if cylinder_err > 1e2:  # 오류 큰 경우만 제외
            cylinder_err = np.inf
        pred_mesh.triangle_normals = o3d.utility.Vector3dVector([])
        o3d.io.write_triangle_mesh("tmp.obj", pred_mesh)
        pred_mesh = pv.read("tmp.obj")
...
from point2cad.fitting_one_surface import process_one_surface  # 반드시 import 필요

def start_timer():
    start_time = time.time()
    while not done_flag[0]:
        elapsed = int(time.time() - start_time)
        print(f"\r⏳ 실행 중... 경과 시간: {elapsed}초", end="")
        time.sleep(1)


def process_only_cylinder(label, points, labels, cfg, device):
    in_points = points[labels == label]
    if len(in_points) < 20:
        return None
    ...
    # Cylinder fitting만 수행
    a, center, radius = fitting.fit_cylinder(
        in_points,
        normals=torch.zeros_like(in_points),
        weights=torch.ones_like(in_points)[:, :1],
        ids=None,
    )

    cylinder_err = np.abs(
        (distance_line_point(center, a, in_points.detach().cpu().numpy()) - radius)
    ).mean()
...
if __name__ == "__main__":
    ...
    # fitting 실행
    result_info = process_only_cylinder(0, points, labels, cfg, device)
